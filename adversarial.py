...
# ---------------------------------------------------------------------------
# Scénario 5 — DIFFAMATION (slander)
# ---------------------------------------------------------------------------

def slander(n_victims: int = 30, n_pulses: int = 6,
            pulse_magnitude: float = 1.0) -> Scenario:
    """Un adversaire dégrade répétitivement une source dont d'autres dérivent.

    Modèle d'adversaire. L'attaquant contrôle une source `adversary` — par
    exemple un système faiblement attesté qu'il a compromis, ou simplement un
    compte légitime dont il peut déclencher des échecs de contrôle. Il ne
    contrôle AUCUNE des données victimes.

    Objectif de l'attaquant : faire chuter la confiance des données victimes
    sous le seuil de gouvernance, provoquant leur blocage — un déni de service
    sur la gouvernance.

    C'est la surface d'attaque créée par l'amplification de gamma_m : plus la
    propagation négative est forte, plus l'attaque est efficace. Elle n'existe
    ni dans Biba (pas d'amplification) ni dans EBSL (amplification interdite).
    """
    g = DRG()
    timeline: list[tuple[float, str, Evidence]] = []

    # Source contrôlée par l'adversaire.
    g.add_node("adversary", base_rate=0.50, label="source contrôlée",
               truly_affected=False)
    timeline.append((1.0, "adversary", make_evidence(
        "creation_attested", POSITIVE, 1.0)))

    # Victimes : données légitimes, bien attestées, dérivant de la source.
    # AUCUNE n'est réellement compromise.
    for i in range(n_victims):
        dio = f"victim_{i}"
        g.add_node(dio, base_rate=0.50, label="donnée légitime",
                   truly_affected=False)
        g.add_edge("adversary", dio, "derivation")
        for k, ty in enumerate(["creation_attested", "schema_validation",
                                "human_validation", "integrity_check_pass"]):
            timeline.append((2.0 + 0.1 * k, dio, make_evidence(ty, POSITIVE,
                                                               2.0 + 0.1 * k)))

    # Impulsions de dégradation déclenchées par l'adversaire.
    t = 10.0
    for p in range(n_pulses):
        timeline.append((t, "adversary", make_evidence(
            "integrity_check_fail", NEGATIVE, t,
            weight=3.0 * pulse_magnitude, source_ref=f"pulse-{p}")))
        # remédiation apparente : l'attaquant fait remonter la confiance
        # pour pouvoir la refaire chuter au coup suivant
        timeline.append((t + 1.0, "adversary", make_evidence(
            "integrity_check_pass", POSITIVE, t + 1.0,
            weight=3.0 * pulse_magnitude, source_ref=f"recover-{p}")))
        t += 5.0

    timeline.sort(key=lambda x: x[0])
    return Scenario(graph=g, timeline=timeline, compromise_time=10.0,
                    description=f"diffamation, {n_pulses} impulsions")
